...
const ProfilePage = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    reader.onload = async () => {
      ...
      await updateProfile({ profilePic: base64Image, fullName: name, bio }) | I
      navigate('/');
    }
        ...
        <form onSubmit={handleSubmit} className='flex flex-col gap-5 p-10 flex-1'>
          ...
          <label htmlFor="avatar" className='flex items-center gap-3
           cursor-pointer'>
            ...
            <img src={selectedImg ? URL.createObjectURL(selectedImg) : assets.arrow_icon} alt=""
              className={`w - 12 h-12 ${selectedImg && 'rounded-full'}`} /> I
         ...
         border-gray-500 rounded-md focus: outline-none focus:ring-2
          ...
          <button className='bg-gradient-to-r from-purple-400 to-violet-600
              text-white p-2 rounded-full text-lg cursor-pointer' type='submit'>
            Save
          </button>
        </form>
